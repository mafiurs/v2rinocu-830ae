...
const AxieCard = ({ id, auction = null, stats = [], breedCount = 0, pureness = 50, ...rest }) => {
  const _genes = rest?.genes?._genes;
            ...
            <div key={idx} className="flex justify-between">
              <span
                ...
                style={{
                  fontSize: '0.7rem',
                  lineHeight: '1.1rem',
                  color: getAxieColors(_.get(_genes, part)?.d?.cls).light
                }}
              >
                {_.get(_genes, part)?.d?.name}
              </span>
              <span
                className="w-1/3 whitespace-nowrap truncate font-semibold"
                style={{
                  fontSize: '0.7rem',
                  lineHeight: '1.1rem',
                  color: getAxieColors(_.get(_genes, part)?.r1?.cls).light
                }}
              >
                {_.get(_genes, part)?.r1?.name}
              </span>
              <span
                className="w-1/3 whitespace-nowrap truncate font-semibold"
                style={{
                  fontSize: '0.7rem',
                  lineHeight: '1.1rem',
                  color: getAxieColors(_.get(_genes, part)?.r2?.cls).light
                }}
              >
                {_.get(_genes, part)?.r2?.name}
              </span>
            </div>
          ))}
        </div>
        <div className="flex flex-row flex-wrap justify-between overflow-hidden items-baseline">
          <div>
            <span className="truncate text-md font-medium">
              Ξ {auction && Number(formatEther(auction?.currentPrice)).toFixed(3)}
            </span>
            <span
            ...
            >
              ${auction && auction?.currentPriceUSD}
            </span>
          </div>
          <div>
            <span
